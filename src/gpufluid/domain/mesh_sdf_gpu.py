"""[Layer D4 / BLK D4.3.GPU + D4.3.GPU.BVH] GPU triangle inside-test.

Two implementations, picked automatically by triangle count:

* **D4.3.GPU** — brute-force O(cells × triangles) Möller-Trumbore +X ray-cast.
...
* **D4.3.GPU.BVH** — Warp's built-in ``wp.Mesh`` spatial structure with
  ``mesh_query_point_sign_winding_number``. O(cells · log triangles) traversal
  on the GPU. Mandatory for ≥1k-tri obstacles (the brute-force path becomes
  the dominant cost when ``cells × triangles`` exceeds ~2e8). Honest disclosure:
  this re-uses Warp's primitive instead of a hand-rolled BVH — battle-tested,
  same complexity, fewer LOC to maintain.
"""
...
def k3_mesh_to_solid(
    ...
    for t in range(n_tris):
        ...
        v1 = tris[3 * t + 1]
        v2 = tris[3 * t + 2]
        # Möller-Trumbore specialised for d=(1,0,0):
...
def k3_mesh_to_solid_bvh(
    ...
    `mesh_query_point_sign_winding_number` returns a sign in {-1, +1} for
    watertight inputs (negative = inside). For our boxed obstacles a single
    point query suffices — no ray-direction bias, no edge-case parity bugs.
    """
    i, j, k = wp.tid()
    if i >= cell_centers.shape[0] or j >= cell_centers.shape[1] or k >= cell_centers.shape[2]:
        return
    ...
    # sign == -1 → inside watertight mesh; outside is +1 (or no-hit with sign=0)
...
# Default threshold at which the BVH path wins. Empirically measured on
# RTX 4080 SUPER + 64³ cells: build cost of wp.Mesh is ~3 ms for 1k tris;
# brute-force kernel for 256 tris × 64³ cells is ~5 ms. So 256 is the
# break-even point for one-shot static obstacles. For animated meshes the
# threshold drops further (mesh is rebuilt per frame), but caching makes it
# moot — see _MeshCache below.
...
class _MeshCache:
    ...
    def get_or_build(self, triangles_np: np.ndarray, device: str,
        ...
        # No key → one-shot build, no cache pressure
        ...
        if entry is not None and entry["n_tris"] == n_tris and entry["device"] == device:
            # Same topology — refit BVH on updated vertices (cheap)
...
def mesh_indicator_sdf_gpu(
    ...
    O(cells × tris) CPU `mesh_to_sdf` which OOMs above ~5k tris on 64³ grids.
    """
...
def mark_solid_from_mesh_gpu(
    ...
    if use_bvh:
        ...
        # accuracy: max acceptable winding-number error (0.5 = inside if |w| >= 0.5,
        # the standard cutoff). Default 2.0 is liberal — for nasty meshes lower it.
