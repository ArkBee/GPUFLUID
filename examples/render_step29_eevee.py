"""step29 — F3.6 hook refactor macro closure (D4 -> F3 inversion fixed).

The change this video certifies is purely architectural: the solver
(F3) no longer imports per-frame helpers from `domain/*` (D4). The 6
historical layer-rule violations all closed via the F3.6 phases (see
DESIGN.md §3.2.4.2). The whitelist `_KNOWN_LAYER_EXCEPTIONS` is the
empty list, guarded by a hard CI test.

The visual backdrop reuses the step28 whitewater_splash bake — the
scene doesn't matter because the change is architectural; only the
overlay carries the macro-closure information.

    F3.6  -  D4 -> F3 hook refactor closed
    6 layer violations -> 0 (one-PR rule)
    FrameEventQueue (G1.19): D4 publishes, F3 drains
    F3.6 macro CLOSED  -  2026-05-17

Run from inside Blender:

    blender --background --python examples/render_step29_eevee.py -- \\
            --cache out/step28_b3_full \\
            --out   out/step29_eevee_frames

Stitch:

    .venv/Scripts/python -c "import imageio.v2 as io, glob; \\
        w = io.get_writer('out/videos/step29.mp4', fps=24, codec='libx264', \\
                          quality=8, macro_block_size=1); \\
        [w.append_data(io.imread(p)) for p in sorted(glob.glob('out/step29_eevee_frames/f_*.png'))]; \\
        w.close()"
"""
...
def build_scene(cache: Path):
    sc = bpy.data.scenes.get("step29_render") or bpy.data.scenes.new("step29_render")
    bpy.context.window.scene = sc
    for o in list(sc.objects):
        sc.collection.objects.unlink(o)
    for nm in [n for n in bpy.data.materials.keys() if n.startswith("step29_")]:
        bpy.data.materials.remove(bpy.data.materials[nm])
    for nm in [n for n in bpy.data.meshes.keys() if n.startswith("step29_")]:
    ...
    world = bpy.data.worlds.get("step29_world") or bpy.data.worlds.new("step29_world")
    ...
    pivot = bpy.data.objects.new("step29_pivot", None)
    ...
    surf_me = bpy.data.meshes.new("step29_surface")
    surf = bpy.data.objects.new("step29_surface", surf_me)
    sc.collection.objects.link(surf)
    surf.parent = pivot
    water = bpy.data.materials.new("step29_water")
    ...
    # step29: whitewater_splash has no obstacle — just a basin + falling jet.

    # Ground plane.
    bpy.ops.mesh.primitive_plane_add(size=4.0, location=(0, 0, 0))
    ground = bpy.context.active_object
    ground.name = "step29_ground"
    gmat = bpy.data.materials.new("step29_ground_mat")
    gmat.use_nodes = True
