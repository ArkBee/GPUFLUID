"""step28 — B3 whitewater macro closure (wave-crest classifier) Eevee renderer.

Whitewater splash scene (examples/scenes/step28_b3_full.toml — same as
step22 but with W7.7 trapped-air AND W7.8 wave-crest blended at β=2.0).
The overlay text reports the B3.4 A/B/C bench numbers measured on the
same scene, where the full classifier shifts ~3pp of emission from
sub-surface (bubble) to surface (foam+spray).

    B3 whitewater: full Ihmsen-2012 classifier
    W7.7 trapped-air + W7.8 wave-crest (beta=2.0)
    surface-fraction +2.5pp / bubble-fraction -3.1pp vs trapped-air alone
    B3 macro CLOSED 2026-05-17

Run from inside Blender:

    blender --background --python examples/render_step28_eevee.py -- \\
            --cache out/step28_b3_full \\
            --out   out/step28_eevee_frames

Stitch:

    .venv/Scripts/python -c "import imageio.v2 as io, glob; \\
        w = io.get_writer('out/videos/step28.mp4', fps=24, codec='libx264', \\
                          quality=8, macro_block_size=1); \\
        [w.append_data(io.imread(p)) for p in sorted(glob.glob('out/step28_eevee_frames/f_*.png'))]; \\
        w.close()"
"""
...
def build_scene(cache: Path):
    sc = bpy.data.scenes.get("step28_render") or bpy.data.scenes.new("step28_render")
    bpy.context.window.scene = sc
    for o in list(sc.objects):
        sc.collection.objects.unlink(o)
    for nm in [n for n in bpy.data.materials.keys() if n.startswith("step28_")]:
        bpy.data.materials.remove(bpy.data.materials[nm])
    for nm in [n for n in bpy.data.meshes.keys() if n.startswith("step28_")]:
    ...
    world = bpy.data.worlds.get("step28_world") or bpy.data.worlds.new("step28_world")
    ...
    pivot = bpy.data.objects.new("step28_pivot", None)
    ...
    surf_me = bpy.data.meshes.new("step28_surface")
    surf = bpy.data.objects.new("step28_surface", surf_me)
    sc.collection.objects.link(surf)
    surf.parent = pivot
    water = bpy.data.materials.new("step28_water")
    ...
    # step28: whitewater_splash has no obstacle — just a basin + falling jet.

    # Ground plane.
    bpy.ops.mesh.primitive_plane_add(size=4.0, location=(0, 0, 0))
    ground = bpy.context.active_object
    ground.name = "step28_ground"
    gmat = bpy.data.materials.new("step28_ground_mat")
    gmat.use_nodes = True
