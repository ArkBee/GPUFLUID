"""step27 — B7-alt v1.0 macro closure (sub-dense storage) Eevee renderer.

Same 96^3 PCG waterfall scene as step27 (visual continuity with the
previous perf-overlay demo), baked with `--enable-sub-dense
--enable-cuda-graphs` so the offset-aware kernels actually run. The
overlay text reports the macro's headline numbers, which were measured
on the 256^3 acceptance bench (`tests/test_b7_alt_6_256_bench.py`) —
the 96^3 scene is just a friendly visual backdrop, the *feature* is
the closure status.

    B7-alt v1.0: deferred-dense allocation
    256^3 dam-break: 6.11x memory drop (642 MB saved)
    on-device rebuild 3 ms / sub-dense step 1 ms / graph hit 87.5%
    13 configs bit-exact — macro CLOSED 2026-05-17

Run from inside Blender:

    blender --background --python examples/render_step27_eevee.py -- \\
            --cache out/step27_sub_dense \\
            --out   out/step27_eevee_frames

Stitch:

    .venv/Scripts/python -c "import imageio.v2 as io, glob; \\
        w = io.get_writer('out/videos/step27.mp4', fps=24, codec='libx264', \\
                          quality=8, macro_block_size=1); \\
        [w.append_data(io.imread(p)) for p in sorted(glob.glob('out/step27_eevee_frames/f_*.png'))]; \\
        w.close()"
"""
...
def build_scene(cache: Path):
    sc = bpy.data.scenes.get("step27_render") or bpy.data.scenes.new("step27_render")
    bpy.context.window.scene = sc
    for o in list(sc.objects):
        sc.collection.objects.unlink(o)
    for nm in [n for n in bpy.data.materials.keys() if n.startswith("step27_")]:
        bpy.data.materials.remove(bpy.data.materials[nm])
    for nm in [n for n in bpy.data.meshes.keys() if n.startswith("step27_")]:
    ...
    world = bpy.data.worlds.get("step27_world") or bpy.data.worlds.new("step27_world")
    ...
    pivot = bpy.data.objects.new("step27_pivot", None)
    ...
    surf_me = bpy.data.meshes.new("step27_surface")
    surf = bpy.data.objects.new("step27_surface", surf_me)
    sc.collection.objects.link(surf)
    surf.parent = pivot
    water = bpy.data.materials.new("step27_water")
    ...
    obs.name = "step27_obstacle"
    obs.parent = pivot
    bpy.ops.object.shade_smooth()
    omat = bpy.data.materials.new("step27_obstacle_mat")
    omat.use_nodes = True
    ...
    ground.name = "step27_ground"
    gmat = bpy.data.materials.new("step27_ground_mat")
    gmat.use_nodes = True
