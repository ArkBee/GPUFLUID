...
| S2.6.6   | Per-tile PCG kernels (B4.2, 1.1× @128³)    | S2    | `gpufluid/solvers/solver3d.py`               | impl,test |
...
| F3.7     | B7-alt sub-dense storage (bbox + rebuild + on-device copy + bounds check) | F3 | `gpufluid/solvers/solver3d.py`               | impl,test |
